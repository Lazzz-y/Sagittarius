package io.github.lazzz.sagittarius.common.redisson.aspect;
...
public class LockAspect implements Ordered {
    ...
    public Object around(ProceedingJoinPoint joinPoint, Lock lock) throws Throwable {
        log.debug("开始执行{}方法 | {}", joinPoint.getSignature().getName(), joinPoint.getArgs());
        String lockName = getLockName(lock.name(), joinPoint);
        ...
        var lockInfo = LockInfo
                .builder().name(lockName).lockType(lockType)
                .waitTime(waitTime).leaseTime(leaseTime).timeUnit(timeUnit).build();
        var ilock = lockFactory.getLock(lockType, lockInfo);
        ...
        try {
            ...
            if (currentThreadLock) {
                log.debug("获取{}锁 | 成功 | {}", lock.lockType(), lockName);
                return joinPoint.proceed();
            }
            log.debug("获取{}锁 | 失败 | {}", lock.lockType(), lockName);
            throw new Exception("请稍后再试！");
        } finally {
            if (currentThreadLock) {
                // 确保当前锁（如读锁）在方法执行后释放
                ilock.release();
                log.debug("释放{}锁 | 成功 | 已释放 | {}", lock.lockType(), lockName);
            }
        }
    }
